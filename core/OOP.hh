...
private:
  ...
  boolean operator==(const OBJECT&); // equality operator disabled (uses 'is_equal' instead)
public:
  OBJECT(): ref_count(0) {}
  virtual ~OBJECT() {
    if (ref_count != 0) {
  ...
  boolean is_equal(const OBJECT&) const { return TRUE; }
  virtual UNIVERSAL_CHARSTRING toString() const {
