...
boolean FLOAT_template::match(double other_value, boolean /* legacy */) const
{
  ...
  case VALUE_RANGE:
    return ((!value_range.min_is_present && (!value_range.min_is_exclusive || (value_range.min_is_exclusive && other_value != MINUS_INFINITY))) ||
            (!value_range.min_is_exclusive && value_range.min_value <= other_value) ||
            (value_range.min_is_exclusive && value_range.min_value < other_value)) &&
      ((!value_range.max_is_present && (!value_range.max_is_exclusive || (value_range.max_is_exclusive && other_value != PLUS_INFINITY))) ||
       (!value_range.max_is_exclusive && value_range.max_value >= other_value) ||
       (value_range.max_is_exclusive && value_range.max_value > other_value));
  default:
