...
boolean FLOAT_template::match(double other_value, boolean /* legacy */) const
{
  ...
  case VALUE_RANGE:
    ...
    return (
      // Min boundary is -infinity and (not min exclusive or            (min exclusive               and  the value is larger than -infinity)
      (!value_range.min_is_present && (!value_range.min_is_exclusive || (value_range.min_is_exclusive && other_value != MINUS_INFINITY))) ||
      ...
      (value_range.min_is_present && value_range.min_is_exclusive && value_range.min_value < other_value))
      &&
      // Max boundary is infinity  and  (not min exclusive or            (max exclusive               and the value is smaller than infinity)
      ((!value_range.max_is_present && (!value_range.max_is_exclusive || (value_range.max_is_exclusive && other_value != PLUS_INFINITY))) ||
