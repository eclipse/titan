    ...
    while (lastest && lastest->project_name != NULL) { // Go through the projects again n^2 complexity
      ...
      while (lastest_child && lastest_child->str != NULL) {
        if (strcmp(last->project_name, lastest_child->str) == 0) { // If a project is a child of another project
          // Add the another project to the project's dependencies
          // But first check if there is already in the dependencies
...
static boolean insert_to_required_config(const struct config_struct* all_configs, const char* project_name, const char* project_config, struct string2_list* const required_configs) {
  ...
  // We if the project's configuration is not already present in the required
...
static boolean insert_to_tmp_config(struct config_list* const tmp_configs, const char* project_name, const char* project_config, const boolean is_active) {
  ...
  while(last && last->str1 != NULL && last->str2 != NULL) {
    if (strcmp(last->str1, project_name) == 0) {
      ...
      if (found_project && strcmp(last->str2, project_config) == 0) {
        found_config = TRUE;
        break;
      }
    }
...
// act_project is the next project which maybe will in be the circle
...
static boolean anal_child(struct config_struct* const all_configs, const char* project_name, const char* project_config, struct string2_list* const required_configs, struct config_list* const tmp_configs) {
  boolean result = TRUE;
  ...
  if (act_config != NULL) {
    ...
    while (last_child && last_child->str != NULL) {
      result = anal_child(all_configs, last_child->str, NULL, required_configs, tmp_configs);
    ...
    while (last && last->requirements != NULL && found == FALSE) {
      ...
      while (req_config && req_config->str1 != NULL && req_config->str2 != NULL) {
        ...
        if (strcmp(req_config->str1, project_name) == 0) {
          ...
          // Insert the project_name with it's active configuration
          ...
          result = anal_child(all_configs, last->project_name, last->project_conf, required_configs, tmp_configs);
          remove_from_tmp_config(tmp_configs, project_name, act_config);
          // If some errors happened during the analsis of last->project_conf project
          if (result == FALSE) {
          ...
          if (get_act_config(required_configs, project_name) == NULL) {// Is this needed?
            // Insert the project with the active configuration
            result = insert_to_required_config(all_configs, project_name, act_config, required_configs);
            if (result == FALSE) return result;
          }
  ...
  while (last) {
    if (last->project_name != NULL && all_configs->project_name != NULL) {
      if (strcmp(all_configs->project_name, last->project_name) == 0 && last->is_active == TRUE && last->processed == FALSE) {
        ...
        struct string_list* last_child = last->children;
        while (last_child && last_child->str != NULL) {
          result = anal_child(all_configs, last_child->str, NULL, required_configs, *tmp_configs); // todo check if everywhere is handled
          if (result == FALSE) return TPD_FAILED;
          last_child = last_child->next;
        }
...
extern "C" tpd_result process_tpd(const char *p_tpd_name, const char *actcfg,
  ...
  if (success == TPD_SUCCESS) {
    ...
    // This round does get the information form the tpd to generate the makefile.
  ...
  *abs_work_dir_p = NULL;
  ...
  if (!get_config_mode) {
    ...
    while (last_elem && last_elem->str1 != NULL && last_elem->str2 != NULL) {
      if (!strcmp(last_elem->str1, *p_project_name) && (strcmp(last_elem->str2, actcfg) || strcmp(last_elem->str2, actcfg))) {
