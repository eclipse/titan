...
static void executeMakefileScript(const char* makefileScript, const char* output_file) {
  ...
  char* tmp_output_file_name = NULL;
  tmp_output_file_name = mputprintf(tmp_output_file_name, "%s.tmp", output_file_name);
  
  char* command = NULL;
  command = mputprintf(command, "%s %s %s", makefileScript, output_file_name, tmp_output_file_name);
  ...
  if(access(output_file_name, W_OK) == -1) {
    ERROR("%s does not exists needed by makefile modifier script", output_file_name);
    goto free_str;
  }
  if(access(tmp_output_file_name, W_OK) == -1) {
    ERROR("%s does not exists needed by makefile modifier script", tmp_output_file_name);
    goto free_str;
  }
