...
error:
  ...
  if (port_op.r.redirect.param != NULL &&
    ...
    char* sig_op = mprintf("%s_call",
      signature->get_genname_value(my_sb).c_str());
    expr->preamble = port_op.r.redirect.param->generate_code_decoded(
      expr->preamble, signature, sig_op, tmp_id.c_str(), false);
    expr->expr = mputprintf(expr->expr, ", %s_redirect_%s(", sig_op, tmp_id.c_str());
    Free(sig_op);
  }
  else {
  ...
  if (port_op.r.redirect.param != NULL &&
    ...
    // redirects have the '@decoded' modifier
    string tmp_id = my_sb->get_scope_mod_gen()->get_temporary_id();
    char* sig_op = mprintf("%s_reply",
      signature->get_genname_value(my_sb).c_str());
    expr->preamble = port_op.r.redirect.param->generate_code_decoded(
      expr->preamble, signature, sig_op, tmp_id.c_str(), true);
    expr->expr = mputprintf(expr->expr, ", %s_redirect_%s(", sig_op, tmp_id.c_str());
    Free(sig_op);
  }
  else {
                                             ...
                                             const char* tmp_id, bool is_out)
  {
    ...
    str = mputprintf(str,
      "class %s_redirect_%s : public %s_redirect {\n"
      ...
      "virtual void set_parameters(const %s& par) const\n"
      "{\n"
      "%s" 
      "%s_redirect::set_parameters(par);\n"
      "}\n"
      "};\n", sig_op, tmp_id, sig_op, members_str,
      sig_op, tmp_id, constr_params_str,
      sig_op, base_constr_params_str, constr_init_list_str,
      sig_op, set_params_str, sig_op);
    Free(members_str);
