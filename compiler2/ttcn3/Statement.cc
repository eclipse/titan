    ...
    for(size_t i=0; i<select_union.sus->get_nof_sus(); i++) {
      ...
      for(size_t j=0; j<size; j++) {
        ...
        if (!t_gov->has_comp_withName(*id)) {
          select_union.expr->error("In the %lu. branch: '%s' is not an alternative of union type '%s'", i+1, id->get_ttcnname().c_str(), t_gov->get_typename().c_str());
          continue;
        }
        for(size_t i2=i; i2<select_union.sus->get_nof_sus(); i2++) {
          ...
          for(size_t j2=j; j2<size; j2++) {
            ...
            if (id->get_ttcnname() == id2->get_ttcnname()) {
              select_union.expr->error("The '%s' is already present in the %lu. branch of select union", id->get_ttcnname().c_str(), i+1);
            }
          }
        }
      }
...
error:
  ...
  char* Statement::generate_code_select_union(char *str)
  {
    ...
    const char* type_name = select_union.expr->get_expr_governor_last()->get_genname_own().c_str();
    char* loc = NULL;
