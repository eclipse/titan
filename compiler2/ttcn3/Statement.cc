...
error:
    ...
    GovernedSimple::code_section_t p_code_section)
  {
    ...
    case S_UNMAP:
      ...
      /*if (config_op.parsed_params != NULL) {
        config_op.parsed_params->set_code_section(p_code_section);
      }*/
      break;
    case S_START_TIMER:
  ...
  char *Statement::generate_code_configop(char *str, const char *opname)
  {
    ...
    if (config_op.ap_list != NULL) {
      ...
      for (size_t i = 0; i < nof_pars; ++i) {
        ...
        bool copy_needed = fp->get_asstype() == Definition::A_PAR_VAL ||
          fp->get_asstype() == Definition::A_PAR_VAL_IN;
        expression_struct par_expr;
        Code::init_expr(&par_expr);
        config_op.ap_list->get_par(i)->generate_code(&par_expr, copy_needed, fp);
