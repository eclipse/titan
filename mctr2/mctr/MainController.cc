...
void MainController::configure_mtc()
{
  ...
  if (mtc->tc_state != TC_IDLE) {
    error("MainController::configure_mtc(): MTC is in wrong state (%d).", mtc->tc_state);
  }
  else {
...
void MainController::set_kill_timer(double timer_val)
{
  ...
  case MC_RECONFIGURING:
    ...
    break;
  default:
    error("MainController::set_kill_timer: called in wrong state (%d).", mc_state);
    break;
  }
  unlock();
}
...
void MainController::configure(const char *config_file)
{
  ...
  case MC_RECONFIGURING:
    break;
  default:
    error("MainController::configure: called in wrong state (%d).", mc_state);
    unlock();
    return;
  }
  Free(config_str);
...
bool MainController::start_reconfiguring()
{
  ...
  default:
    lock();
    error("MainController::start_reconfiguring: called in wrong state (%d).", mc_state);
    unlock();
    return false;
  }
}
